  ...
  <div class="bg-white py-4 px-6 w-full h-full absolute inset-0">
    <div class="flex content-center items-center mb-4 justify-between">
      ...
      <el-button
        ...
        circle
        type="default"
        class="shadow-md focus:shadow-none focus:outline-none"
      ></el-button>
    </div>
    <el-form :model="settingForm" label-width="120px">
      <el-form-item :label="text.language">
        ...
        <p class="text-gray-400 text-xs my-1.5">{{ text.languageHint }}</p>
      </el-form-item>
      ...
      <el-form-item :label="text.autoUpdate">
        <el-switch @change="saveSetting" v-model="settingForm.autoUpdate"></el-switch>
      </el-form-item>
      <el-form-item :label="text.exportData">
        <el-button @click="exportUIGFJSON" type="success" plain class="focus:outline-none">{{
          text.exportJSON
        }}</el-button>
      </el-form-item>
    </el-form>
    ...
    <p class="text-gray-600 text-xs mt-1">
      Github: <Link href="https://github.com/arkntools/arknights-gacha-export" />
    </p>
    <p class="text-gray-600 text-xs mt-1 pb-6">Origin: <Link href="https://github.com/biuuu/genshin-wish-export" /></p>
  </div>
</template>
...
const saveSetting = async () => {
  const keys = ['lang', 'logType', 'autoUpdate']
  for (let key of keys) {
