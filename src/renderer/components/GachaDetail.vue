<template>
  ...
  <p class="text-gray-600 text-xs mb-1">
    <span class="mr-1"
      >{{ text.total }} <span class="text-blue-600">{{ detail.total }}</span> {{ text.times }}</span
    >
    <span
      >{{ text.sum }}<span class="mx-1 text-green-600">{{ detail.countMio }}</span
      >{{ text.no6star }}</span
    >
  </p>
  <p class="text-gray-600 text-xs mb-1">
    <span class="mr-3 whitespace-pre text-red-600">
      <span class="min-w-10 inline-block">{{ text.star6 }}{{ colon }}{{ detail.count6 }}</span>
      [{{ percent(detail.count6, detail.total) }}]
    </span>
    <br />
    <span class="mr-3 whitespace-pre text-orange-600">
      <span class="min-w-10 inline-block">{{ text.star5 }}{{ colon }}{{ detail.count5 }}</span>
      [{{ percent(detail.count5, detail.total) }}]
    </span>
    <br />
    <span class="mr-3 whitespace-pre text-purple-600">
      <span class="min-w-10 inline-block">{{ text.star4 }}{{ colon }}{{ detail.count4 }}</span>
      [{{ percent(detail.count4, detail.total) }}]
    </span>
    <br />
    <span class="whitespace-pre text-green-600">
  ...
  <p class="text-gray-600 text-xs mb-1" v-if="detail.ssrPos.length">
    {{ text.history }}{{ colon }}
    <span class="mr-1" :style="`color:${colorList[index]}`" v-for="(item, index) of detail.ssrPos" :key="item">
      {{ item[0] }}[{{ item[1] }}]
    </span>
  </p>
  <p v-if="detail.ssrPos.length" class="text-gray-600 text-xs">
    {{ text.average }}{{ colon }}<span class="text-green-600">{{ avg6(detail.ssrPos) }}</span>
  </p>
</template>
...
const colorList = computed(() => {
  ...
  const result = []
  const map = new Map()
  props.data.ssrPos.forEach((item) => {
    if (map.has(item[0])) {
